...
export class LogsComponent implements OnInit {
  ...
  loadCustomers(event?: LazyLoadEvent) {
    ...
    this.itemService.getAllLogs(body).subscribe((res) => {
  ...
  columnsHeader() {
    ...
    let columns = [
      ...
      { field: 'modifiedField', header: 'Modifeid Field' },
      { field: 'modifiedValue', header: 'Modified value' },
      { field: 'operationDate', header: 'Operation Date', type: 'date', format: `MM/dd/yyyy hh:mm a`, data: true },
  ...
  getShortCutStatus() {
    this.dtMmgmtService.getSortcutName('Logs').subscribe((res) => {
      if (res.statusCode == 200) {
